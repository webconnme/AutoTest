...
func ThreadCheckMsg() {
	...
	for !ThreadCheckMsgReqEnd {
		...
		for !ThreadCheckMsgReqEnd {
			...
			event_count, err := zmq.Poll(pi, 100 * time.Millisecond)
			if err != nil {
				ad.Println("fail do not poll[%s]", err)
				reason := fmt.Sprintf("do not poll[%s]", err)
				ar.SetResultError(reason)
			...
			if event_count == 0 {
				...
				if pi[0].REvents & zmq.POLLIN != 0 {
					...
					concatBuf = append(remainBuffer, buf...)
					remainBuffer = []byte{}
			...
			for index, b := range concatBuf {
				if checkSameMessage(b) {
					currentMsg = []byte{}
					cmdIndex++

					if index < len(concatBuf) {
						remainBuffer = concatBuf[(index + 1):]
					}
					fmt.Printf("Remain buffer: [%s]\n", string(remainBuffer))
					break
				}
