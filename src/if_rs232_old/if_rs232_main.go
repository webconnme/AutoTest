...
var PortRxPUB   *zmq.Socket = nil        //  수신 데이터 발송 PUB 소켓 
...
//	}
	...
	if link_port ==  "RX DATA" {
	    jmclog.LogWrite( "Connect PORT [RX DATA]\n" )
	    ...
	    PortRxPUB, err = zmqContext.NewSocket(zmq.PUB)
	    if err != nil {
	        jmclog.LogWrite( "fail New Socket PortRxPUB  Error [%s]\n", err );
	    	return
	    }
	    
	    // 채널에 바인드 한다. 
	    PortRxPUB.Bind("ipc:///tmp/jmc_channel" + link_channel )
...
func main() {
    ...
    jmclog.LogWrite( "if_rs232 component start" );
	
	component_class     := "if_rs232"
	var component_option    []byte
	
	var err error
	
	var argv_options map[string]interface{}
	var serial_device_path string
	...
	if len(os.Args) > 2 {
	   component_option     = []byte(os.Args[2])
	} 
	...
	err = json.Unmarshal(component_option, &argv_options)
	if err != nil {
	    jmclog.LogWrite( "fail Convert Argv Option JSON [%s]", err )
		goto error_no_argument
	}
	jmclog.LogWrite("%+v", argv_options)
	
	serial_device_path = argv_options["dev"].(string)
    jmclog.LogWrite( "serial_device_path = [%s]", serial_device_path );

	// RS232 포트를 연다. 
	OpenRS232( serial_device_path )
	go MainRxRS232()
