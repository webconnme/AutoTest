...
func main() {
	...
	RS232Path = argv_options["dev"].(string)
    ad.Println( "RS232 path = [%s]", RS232Path )
	OpenRS232()
...
func CallbackStart( af *at.AtFrame )(bool){
	...
	for {
	    time.Sleep( time.Millisecond )  
	    if ThreadRS232TxRun && ThreadRS232RxRun {
		    break
		}
	}
...
func CallbackStop( af *at.AtFrame )(bool){
	...
	ThreadRS232RxReqEnd = true
	ThreadRS232TxReqEnd = true
